...
static void parse_unsigned_correct(void) {
    ...
    size_t parsed;
    char const* tail;
    qc_assert(qc_str_to_unsigned(input, &parsed, &tail) == QC_SUCCESS, "qc_str_to_unsigned has failed");
    size_t expected = 567;
...
static void parse_unsigned_ignore_null_tail(void) {
    char const* input = "567";
    size_t parsed;
    qc_assert(qc_str_to_unsigned(input, &parsed, NULL) == QC_SUCCESS, "qc_str_to_unsigned has failed");
    size_t expected = 567;
    qc_assert(parsed == expected, "Expected: %zu, got: %zu", expected, parsed);
}
...
static void parse_unsigned_negative_sign(void) {
    ...
    size_t parsed;
    qc_assert(qc_str_to_unsigned(input, &parsed, NULL) == QC_FAILURE, "qc_str_to_unsigned is expected to fail");
}
