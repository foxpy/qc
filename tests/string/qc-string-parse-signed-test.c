...
static void parse_unsigned_correct(void) {
    ...
    ptrdiff_t parsed;
    char const* tail;
    qc_assert(qc_str_to_signed(input, &parsed, &tail) == QC_SUCCESS, "qc_str_to_signed has failed");
...
static void parse_signed_ignore_null_tail(void) {
    char const* input = "567";
    ptrdiff_t parsed;
    qc_assert(qc_str_to_signed(input, &parsed, NULL) == QC_SUCCESS, "qc_str_to_signed has failed");
    ptrdiff_t expected = 567;
    qc_assert(parsed == expected, "Expected: %td, got: %td", expected, parsed);
}
...
static void parse_signed_garbage(void) {
    ...
    ptrdiff_t parsed;
    qc_assert(qc_str_to_signed(input, &parsed, NULL) == QC_FAILURE, "qc_str_to_signed is expected to fail");
}
