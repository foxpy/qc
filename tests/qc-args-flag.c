...
void test_flag_short() {
    ...
    bool ret = qc_args_parse(args, 2, (char*[]){"/path/to/exe", "-v", NULL}, &err);
    qc_assert(ret, "qc_args_parse has failed");
...
void test_flag_short_multiple() {
    ...
    bool ret = qc_args_parse(args, 3, (char*[]){"/path/to/exe", "-ab", "-c", NULL}, &err);
    qc_assert(ret, "qc_args_parse has failed");
...
void test_flag_long() {
    ...
    qc_args* args = qc_args_new();
    qc_args_flag(args, 'v', "verbose", &verbose, NULL);
    bool ret = qc_args_parse(args, 2, (char*[]){"/path/to/exe", "--verbose", NULL}, &err);
    qc_assert(ret, "qc_args_parse has failed");
    qc_assert(verbose == true, "expected: true");
    qc_args_free(args);
}
...
void test_flag_none() {
    ...
    qc_args* args = qc_args_new();
    qc_args_flag(args, 'v', "verbose", &verbose, NULL);
    bool ret = qc_args_parse(args, 1, (char*[]){"/path/to/exe", NULL}, &err);
    qc_assert(ret, "qc_args_parse has failed");
